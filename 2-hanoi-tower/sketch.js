...
function setup() {
  ...
  slider = createSlider(0.5, 15, 1, 0.5);
  slider.position(windowWidth / 4, windowHeight * 0.8);

  button = createButton('Simülasyonu Başlat');
  button.position(windowWidth/4, 20);
  button.mousePressed(changeSimState);
...
function changeSimState(){
  simState = !simState;
  if(simState){
    button.html('Simülasyonu Durdur');
  }else{
    button.html('Simülasyonu Başlat');
  }
}
...
function draw() {
  ...
  textAlign(CENTER);
  textSize(12);
  text('Simülasyon hızını değiştir: ' + slider.value(), windowWidth/2, windowHeight * 0.8);
